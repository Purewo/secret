...
API Key 不能新建保险柜或 Skill 分类，即使拥有“全部分类”权限也不行；分类只能由管理员在 Web 管理台创建。管理员为 Key 指定分类时，除明确选择“全部分类”外，只能选已存在的分类。Key 可以在获准的现有分类中新增条目；移动已有条目需要对原分类有读取权限、对目标分类有新增权限，未知分类不会被自动创建。
